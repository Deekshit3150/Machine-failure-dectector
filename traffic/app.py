...
with col2:
    air_temperature = st.number_input('🌡 Air Temperature [K]', min_value=250, max_value=400, value=300)
...
with col1:
    torque = st.number_input('⚙ Torque [Nm]', min_value=0, max_value=100, value=50)
...
if st.button('🔍 Predict Failure'):
    ...
    if failure_pred[0] == 1:
        st.error("⚠ Machine will suffer Failure")
        ...
        if torque > 60:
            failure_reasons.append("⚙ *High Torque*: Excessive load on machine components.")
        ...
        if air_temperature > 310:
            failure_reasons.append("🌡 *High Air Temperature*: Inefficient cooling.")
        ...
        st.write("""- 🛠 *Regularly inspect and maintain machine components.*  
                    - ⚙ *Reduce excessive torque and ensure proper lubrication.*  
                    - 🌡 *Monitor and control process temperature to avoid overheating.*  
                    - 🔧 *Replace worn-out tools before they cause damage.*  
                    - ❄ *Maintain an optimal air temperature for efficient operation.*""")
